...
const mobileVoiceInput = document.getElementById('mobileVoiceInput');
...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', function () {
        ...
        document.getElementById('documentationTab').style.display = tab === 'documentation' ? 'block' : 'none';
...
// 修改 formatOutput 函数，添加Markdown图片支持
function formatOutput(output) {
    ...
    // 处理Markdown图片语法
    const withImages = decodedOutput.replace(
        /!\[(.*?)\]\((.*?)\)/g,
        '<div class="output-image-container">' +
            '<img src="$2" alt="$1" class="output-image" onclick="showImageModal(this.src, this.alt)">' +
...
function processAiContent(content) {
    ...
    processedContent = processedContent.replace(
        ...
        '<div class="markdown-image-container">' +
            '<img src="$2" alt="$1" class="markdown-image" onclick="showImageModal(this.src, this.alt)">' +
            '<div class="image-caption">$1</div>' +
        '</div>'
    );
    ...
    // 处理水平线
    processedContent = processedContent.replace(/^\s*---\s*$/gm, '<hr>');
...
function addMessage(sender, content, type) {
    ...
    setTimeout(() => {
        ...
        addCopyButtonToCodeBlocks();
        // 为图像添加点击事件
        addImageClickEvents(messageDiv);
    }, 100);
}

function addImageClickEvents(messageDiv) {
    // 为手册图像添加点击事件
    messageDiv.querySelectorAll('.handbook-image').forEach(img => {
        img.addEventListener('click', function() {
            const title = this.closest('.result-image')?.querySelector('.image-title')?.textContent || this.alt || '图像';
            showImageModal(this.src, title);
        });
    });
    
    // 为用户上传的图像添加点击事件
    messageDiv.querySelectorAll('.uploaded-image').forEach(img => {
        img.addEventListener('click', function() {
            showImageModal(this.src, this.alt || '上传的图像');
        });
    });
    
    // 为Markdown图像添加点击事件
    messageDiv.querySelectorAll('.markdown-image, .output-image').forEach(img => {
        img.addEventListener('click', function() {
            showImageModal(this.src, this.alt || 'Markdown图像');
        });
    });
    
    // 为所有图像添加点击事件（通用）
    messageDiv.querySelectorAll('img[src^="data:image"], img[src^="http"], img[src^="/"]').forEach(img => {
        if (!img.classList.contains('handbook-image') && 
            !img.classList.contains('uploaded-image') && 
            !img.classList.contains('markdown-image') &&
            !img.classList.contains('output-image')) {
            img.addEventListener('click', function() {
                showImageModal(this.src, this.alt || '图像');
            });
        }
    });
}
...
function initImageUpload() {
    ...
    if (mobileImageUpload) {
        mobileImageUpload.addEventListener('click', function() {
            if (!isLoggedIn) {
                showLoginModal();
                return;
            }
    ...
    // 关闭上传界面
    closeUploadBtn.addEventListener('click', closeImageUpload);
    cancelUploadBtn.addEventListener('click', closeImageUpload);
...
function handleImageFile(file) {
    ...
    if (!allowedTypes.includes(file.type)) {
        alert('不支持的文件格式。请选择图像文件 (JPEG, PNG, GIF, BMP, WebP)');
        return;
    }
                ...
                const imageHtml = `
                    <div class="uploaded-image-container">
                        <img src="${e.target.result}" 
                             ...
                             onclick="showImageModal(this.src, '上传的图像')">
                        <div class="image-caption">上传的图像 (${formatFileSize(selectedImageFile.size)})</div>
                    </div>
                `;
                addMessage('user', imageHtml, 'html');
                
                // 显示提问输入框，让用户可以输入问题
                showQuestionInputWithImage(base64Data);
                closeImageUpload();
            };
...
// 显示带图像提问的输入框
function showQuestionInputWithImage(imageBase64) {
    // 创建提问输入区域
    const questionContainer = document.createElement('div');
    questionContainer.className = 'image-question-container';
    questionContainer.innerHTML = `
        <div class="image-question-header">
            <i class="fas fa-image"></i>
            <span>请描述您的问题（与上传的图像相关）</span>
        </div>
        <div class="image-question-input">
            <textarea id="imageQuestionInput" placeholder="请输入与图像相关的问题..."></textarea>
            <div class="image-question-actions">
                <button id="sendImageQuestion" class="btn-primary">
                    <i class="fas fa-paper-plane"></i> 发送问题
                </button>
                <button id="cancelImageQuestion" class="btn-secondary">
    ...
    // 添加事件监听器
    document.getElementById('sendImageQuestion').addEventListener('click', function() {
        sendQuestionWithImage(imageBase64);
    });
    
    document.getElementById('cancelImageQuestion').addEventListener('click', function() {
        questionContainer.remove();
    });
    
    // 自动聚焦到输入框
    const imageQuestionInput = document.getElementById('imageQuestionInput');
    imageQuestionInput.focus();
    
    // 支持回车发送
    imageQuestionInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault();
            sendQuestionWithImage(imageBase64);
        }
...
// 发送带图像的问题
function sendQuestionWithImage(imageBase64) {
    ...
    // 移除问题输入区域
    const questionContainer = document.querySelector('.image-question-container');
    if (questionContainer) {
        questionContainer.remove();
    }
    
    // 显示用户的问题
    addMessage('user', question + ' (包含图像)', 'text');
...
function showImageModal(imageSrc, title) {
    ...
    modalImageTitle.textContent = title || '查看图像';
...
function downloadModalImage() {
    const fileName = modalImageTitle.textContent || 'image';
    downloadImage(modalImage.src, fileName + '.png');
}
...
function initDocumentation() {
    // 标签切换时加载文档列表
    document.querySelectorAll('.nav-item[data-tab="documentation"]').forEach(item => {
        item.addEventListener('click', function() {
    ...
    // 搜索按钮事件
    docSearchBtn.addEventListener('click', searchMarkdown);
    docSearchInput.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            searchMarkdown();
        }
    });
    ...
    // 刷新按钮事件
    docRefreshBtn.addEventListener('click', function() {
        loadMarkdownFiles();
    });
...
function loadMarkdownFiles() {
    docList.innerHTML = '<div class="loading-docs"><i class="fas fa-spinner fa-spin"></i> 加载文档列表中...</div>';
    
    fetch('/get_markdown_files')
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            ...
            // 更新统计数据
            totalDocs.textContent = data.total_files;
            totalImages.textContent = data.total_images;
            
            // 显示文件列表
            renderDocList(markdownFiles);
            
            // 如果当前在查看文档，刷新文档列表后保持当前文档
            if (currentDocPath) {
                const currentFile = markdownFiles.find(f => f.path === currentDocPath);
                if (currentFile) {
                    highlightCurrentDoc(currentFile.path);
                }
            }
        } else {
            docList.innerHTML = `<div class="error-message">加载失败: ${data.error}</div>`;
        }
    })
    .catch(error => {
        docList.innerHTML = `<div class="error-message">网络错误: ${error.message}</div>`;
    });
}
...
function renderDocList(files) {
    if (files.length === 0) {
        docList.innerHTML = '<div class="no-docs">没有找到Markdown文档</div>';
        return;
    }
    
    let html = '<div class="doc-category">';
    let currentDir = '';
    ...
    files.forEach(file => {
        ...
        // 如果目录变化，添加目录标题
        if (dirPath !== currentDir && dirPath) {
            if (currentDir !== '') {
                html += '</div>'; // 关闭上一个目录组
            }
            html += `<div class="doc-directory">
        ...
        const hasImages = file.has_images ? '<i class="fas fa-image has-images" title="包含图片"></i>' : '';
        
        html += `<div class="doc-item" data-path="${file.path}">
                    <div class="doc-item-icon">
                        <i class="fas fa-file-alt"></i>
                    </div>
                    <div class="doc-item-content">
                        <div class="doc-item-name">${escapeHtml(file.name)} ${hasImages}</div>
                        <div class="doc-item-meta">
                            <span class="doc-item-size">${fileSize}</span>
                            <span class="doc-item-path">${file.path}</span>
                        </div>
                    </div>
                </div>`;
    });
...
function loadMarkdownContent(filePath) {
    ...
    // 显示加载状态
    docViewer.innerHTML = '<div class="loading-content"><i class="fas fa-spinner fa-spin"></i> 加载文档内容...</div>';
    docPath.textContent = `正在加载: ${filePath}`;
    docBackBtn.style.display = 'inline-block';
    
    // 高亮当前选中的文档
    highlightCurrentDoc(filePath);
    
    fetch(`/get_markdown_content/${encodeURIComponent(filePath)}`)
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            // 显示文档内容
            renderMarkdownContent(data.content, data.path, data.images);
            docPath.textContent = data.path;
            
            // 滚动到顶部
            docViewer.scrollTop = 0;
        } else {
            docViewer.innerHTML = `<div class="error-message">加载失败: ${data.error}</div>`;
        }
    })
    .catch(error => {
        docViewer.innerHTML = `<div class="error-message">网络错误: ${error.message}</div>`;
    });
}

// 渲染Markdown内容（修改后的版本，添加LaTeX支持）
function renderMarkdownContent(content, filePath, images) {
    // 检查是否已加载markdown-it
    if (typeof window.markdownit === 'undefined') {
        docViewer.innerHTML = `
            <div class="error-message">
                <p>Markdown渲染库未加载，请刷新页面重试。</p>
                <p>如果问题持续存在，请检查网络连接。</p>
            </div>
        `;
        return;
    }
    ...
    const processedContent = content.replace(/!\[(.*?)\]\((.*?)\)/g, (match, alt, src) => {
        ...
        // 如果是相对路径，查找对应的图片
        const imageKey = findImageByPath(src, baseDir, images);
        if (imageKey) {
            return `![${alt}](${imageKey})`;
        }
    ...
    docViewer.innerHTML = `
        <div class="markdown-content">
            ...
            <div class="doc-body" id="markdownBody">
                ${html}
            </div>
            ${images.length > 0 ? `
            <div class="doc-images">
                <h3><i class="fas fa-images"></i> 文档图片 (${images.length}张)</h3>
    ...
    // 为图片添加点击事件
    docViewer.querySelectorAll('img').forEach(img => {
        img.addEventListener('click', function() {
    ...
    // 为链接添加target="_blank"
    docViewer.querySelectorAll('a').forEach(link => {
    ...
    if (typeof renderMathInElement !== 'undefined') {
        renderMathInElement(document.getElementById('markdownBody'), {
            delimiters: [
                ...
                {left: '\\(', right: '\\)', display: false}, // LaTeX 风格
                {left: '\\[', right: '\\]', display: true}   // LaTeX 风格块级
            ],
            throwOnError: false,
...
// 查找图片
function findImageByPath(imgPath, baseDir, images) {
    // 尝试匹配图片
    for (const img of images) {
        if (img.url && img.url.includes(imgPath)) {
            return img.base64 || img.url;
        }
        if (img.path && img.path.includes(imgPath)) {
            return img.base64 || img.url;
        }
    }
...
// 高亮当前选中的文档
function highlightCurrentDoc(filePath) {
    docList.querySelectorAll('.doc-item').forEach(item => {
        item.classList.remove('active');
...
function showDocList() {
    currentDocPath = null;
    docBackBtn.style.display = 'none';
    docPath.textContent = '请选择左侧文档进行查看';
    docViewer.innerHTML = `
        <div class="welcome-doc">
            ...
            <div class="doc-stats">
                <div class="stat-item">
                    <i class="fas fa-file-alt"></i>
                    <span>${markdownFiles.length}</span> 个文档
                </div>
                <div class="stat-item">
                    <i class="fas fa-images"></i>
                    <span>${document.getElementById('totalImages').textContent}</span> 张图片
                </div>
            </div>
        </div>
    `;
...
function searchMarkdown() {
    ...
    docViewer.innerHTML = '<div class="loading-content"><i class="fas fa-spinner fa-spin"></i> 搜索中...</div>';
    docPath.textContent = `搜索结果: ${query}`;
    docBackBtn.style.display = 'inline-block';
    
    fetch('/search_markdown', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        if (data.success) {
            renderSearchResults(data.results, query, data.total_matches);
        } else {
            docViewer.innerHTML = `<div class="error-message">搜索失败: ${data.error}</div>`;
        }
    })
    .catch(error => {
        docViewer.innerHTML = `<div class="error-message">网络错误: ${error.message}</div>`;
...
// 渲染搜索结果
function renderSearchResults(results, query, totalMatches) {
    if (results.length === 0) {
        docViewer.innerHTML = `
            <div class="no-results">
                <i class="fas fa-search"></i>
                <h3>未找到相关结果</h3>
                <p>没有找到包含 "${query}" 的文档内容</p>
            </div>
        `;
        return;
    }
        ...
        <div class="search-results">
            <div class="results-header">
                ...
                <div class="results-stats">
                    找到 ${totalMatches} 个匹配项，显示 ${results.length} 个
                </div>
            </div>
    `;
    ...
    results.forEach((result, index) => {
        html += `
            <div class="search-result-item" data-path="${result.file}">
                <div class="result-header">
                    <span class="result-index">${index + 1}</span>
                    <span class="result-file">${result.file}</span>
                    <button class="btn btn-sm btn-outline view-source-btn">
    ...
    // 为查看原文按钮添加事件
    docViewer.querySelectorAll('.view-source-btn').forEach((btn, index) => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const filePath = results[index].file;
            loadMarkdownContent(filePath);
        });
    });
    
    // 为搜索结果项添加点击事件
    docViewer.querySelectorAll('.search-result-item').forEach((item, index) => {
        item.addEventListener('click', function() {
            const filePath = results[index].file;
            loadMarkdownContent(filePath);
        });
...
window.addEventListener('load', function() {
    ...
    setTimeout(() => {
        ...
        // 为现有图像添加点击事件
        document.querySelectorAll('.handbook-image, .uploaded-image, img[src^="data:image"]').forEach(img => {
            img.addEventListener('click', function() {
                const title = this.alt || this.closest('.handbook-image-container')?.querySelector('.image-title')?.textContent || '图像';
                showImageModal(this.src, title);
            });
        });
        
        // 初始化页面中的 LaTeX 公式（用于页面加载时已经存在的数学公式）
        if (typeof renderMathInElement !== 'undefined') {
...
function updateLastMessage(content) {
    ...
    if (lastMessage) {
        ...
        setTimeout(() => {
            ...
            addCopyButtonToCodeBlocks();
            // 为新添加的图像添加点击事件
            addImageClickEvents(lastMessage.closest('.message'));
        }, 100);
...
function enhanceQuestionWithHandbook(question) {
    ...
    if (hasBasicConcept) {
        console.log('检测到基础概念问题，将在回答中参考《Python-100-Days》内容');
    }
